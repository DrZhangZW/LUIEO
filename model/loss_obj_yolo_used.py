...
import math
import numpy as np
from utils.common_utils import  check_version
TORCH_1_10 = check_version(torch.__version__, '1.10.0')

class v8DetectionLoss:
    #BboxLoss
    def __init__(self, opts,model):  # model must be de-paralleled
        device = getattr(opts, "dev_device", torch.device('cpu'))
        try:
            m = model.model[-1]
        except:
            m = model.head.Detect_head  # Detect() module
    ...
    def __call__(self, preds, batch):
        ...
        self.stride = []
        for tmp_feat in preds:
            size_img = batch['size'][0]
            stride = torch.div(size_img, tmp_feat.shape[2])
            self.stride.append(stride[0])
        ...
        loss[1] = self.bce(pred_scores, target_scores.to(dtype)).sum() / target_scores_sum  # BCE yolov8
        ...
        loss_obj = loss.sum() * batch_size

        losses = dict()
        losses["loss_obj_yolo"] = loss_obj
        losses["loss_box_yolo"] = loss[0]
        losses["loss_class_yolo"] = loss[1]
        losses["loss_dfl_yolo"] = loss[2]

        return losses  # loss(box, cls, dfl)
